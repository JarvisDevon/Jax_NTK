...
def gen_poly_data(input_dims, inputs_scale, params_scale, batch_size, noise_var, bias=True, transposed=False):
  print("Start load", flush=True)
  if bias:
      powers_x = input_dims[1] - 1
  else:
      powers_x = input_dims[1]
  inputs = np.random.normal(0, inputs_scale, (input_dims[0],1))
  for i in range(powers_x-1):
      inputs = np.hstack([inputs, inputs[:,-1].reshape(input_dims[0],1)*inputs[:,0:1]])
  if bias:
      inputs = np.hstack([np.ones((input_dims[0],1)), inputs])
  gen_params = np.random.normal(0, params_scale, (input_dims[1], 1))
  print("Ground Truth Polynomial Parameters: ", gen_params)
  labels = jnp.dot(inputs, gen_params)
  ...
  if transposed == False:
    ...
    train_images, train_labels, test_images, test_labels = train_images.T, train_labels.T, test_images.T, test_labels.T
    def data_stream():
      rng = npr.RandomState(0)
      while True:
        perm = rng.permutation(num_train)
...
def gen_sin_data(input_dims, inputs_scale, params_scale, outputs_scale, batch_size, noise_var = 1.0, transposed = False):
  print("Start load", flush=True)
